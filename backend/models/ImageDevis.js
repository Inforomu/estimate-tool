...
class Image {
    ...
    static async getImagesForDevis(devisId) {
        const sql = `
        SELECT Image.id, Image.image_data 
        FROM Devis 
        INNER JOIN Devis_Image ON Devis.id = Devis_Image.devis_id 
        INNER JOIN Image ON Devis_Image.image_id = Image.id 
        WHERE Devis.id = ?;
        `;

        try {
            const connection = await db.execute();
            const [results] = await connection.execute(sql, [devisId]);
            return results;
        } catch (error) {
